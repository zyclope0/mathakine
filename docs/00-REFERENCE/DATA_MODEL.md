...
| `role`                      | Enum(UserRole)                 | Stockage DB legacy `padawan / maitre / gardien / archiviste` ; contrat applicatif/API canonique `apprenant / enseignant / moderateur / admin` |
| `is_active`                 | Boolean                        |                                                                                                                                               |
...
Note role utilisateur (phase 1 de migration) :

- La base SQLAlchemy conserve encore l'enum legacy `UserRole`.
- Les services, schemas et payloads API exposent des roles canoniques :
  - `apprenant`
  - `enseignant`
  - `moderateur`
  - `admin`
