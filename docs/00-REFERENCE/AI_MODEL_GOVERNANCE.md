...
- Defis IA : `generation_metrics` expose aussi `generation_status_counts`, `error_code_counts`, percentiles de latence, `fallback_stats`, compteurs de repair chess et `repair_success_rate`; `generation_confidence` est journalise cote orchestration mais non persiste en base.
- Resilience amont OpenAI sur les workloads pedagogiques SSE : `openai_workload_circuit_breaker` partage un etat `CLOSED / OPEN / HALF_OPEN` process-local et intercepte les indisponibilites repetitives avant l'appel OpenAI. Sources : `app/utils/circuit_breaker.py`, `app/services/exercises/exercise_ai_service.py`, `app/services/challenges/challenge_ai_service.py`.
...
- Pour les defis, lire d'abord `generation_status_counts` et `error_code_counts` avant d'ajouter de nouvelles regles : ils indiquent si le pipeline accepte, repare, rejette ou echoue sur un contrat particulier.
...
- Le circuit breaker OpenAI est lui aussi process-local ; il protege un worker donne mais n'assure pas de coordination distribuee multi-instance. Sources : `app/utils/circuit_breaker.py`, `app/services/exercises/exercise_ai_service.py`, `app/services/challenges/challenge_ai_service.py`.
