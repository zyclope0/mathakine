...
| GET | `/api/users/stats` | stats **filtre temporel** (tentatives, réussite, séries, graphiques…) — **sans** XP ni niveau compte ; gamification persistante → `/me` (`gamification_level`, `total_points`, …) ; bloc **`spaced_repetition`** (F04) : `f04_initialized`, `active_cards_count`, `due_today_count`, `overdue_count`, `next_review_date` (ISO ou null) |
