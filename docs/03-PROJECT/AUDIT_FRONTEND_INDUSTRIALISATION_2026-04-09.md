...
La migration globale des ~130 autres fichiers **n’est pas** revendiquée ; **ACTIF-03** reste ouvert. **Terrain 2026-04-12** : la co-localisation des tests **hooks** sous **`frontend/__tests__/unit/hooks/`** est **quasi soldée** (dossier épuisé après ce lot) ; le reste du périmètre **`frontend/__tests__/unit/`** (composants, pages, routes API, etc.) reste hors scope d’un seul lot.
