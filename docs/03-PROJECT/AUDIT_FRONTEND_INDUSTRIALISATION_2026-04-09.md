...
| Score estimé après ACTIF-03 fermé (2026-04-14)  | **~8.0 / 10**            |
| Findings P0 ouverts                             | **0**                    |
| Findings P1 ouverts                             | **0**                    |
| Findings P2 ouverts                             | **1** (ACTIF-04)         |
...
**Verdict :** le frontend est industriellement mature sur TypeScript, cache React Query et guardrails CI. **ACTIF-03 fermé (2026-04-14)** : co-localisation complète — `__tests__/unit/` réduit au seul `architecture/frontendGuardrails.test.ts` (intentionnel). Le risque résiduel est la remontée mesurée de la couverture et des seuils Vitest (D8 / ACTIF-04). Le finding **ACTIF-02** (images dynamiques D7) est **clos** : hybrides `next/image` / `<img>` pour avatar et badges ; **exception documentée** `<img>` pour le chat (**`ChatMessagesView`**).
...
| D5 Modularité            | 8/10  | 15 %      | 1.20          | 0          |
| D6 Maintenabilité        | 7/10  | 15 %      | 1.05          | 0          |
| D7 Performance           | 6/10  | 10 %      | 0.60          | 0          |
| D8 Réplicabilité / Tests | 8/10  | 10 %      | 0.80          | 0          |
| **TOTAL**                |       | **100 %** | **~7.7/10**   | **0**      |

> Les scores D1, D4, D7, D8 reflètent les lots exécutés depuis l'audit initial (pages décomposées, ESLint durci, lazy loading PDF/Excel). D5 remonte à 8/10 après fermeture ACTIF-03 (2026-04-14).
...
> Les findings sont listés du plus prioritaire au moins prioritaire. ACTIF-05 est intentionnellement en dernier : c'est un backlog optionnel. Si Codex cherche quoi faire, il commence par **ACTIF-04**.
...
| ~~ACTIF-03~~ | ~~P2~~ — **FERMÉ** | —             | Co-localisation complète (Phase D commit `905419a`, 2026-04-14) : `__tests__/unit/` réduit au seul `architecture/frontendGuardrails.test.ts` (intentionnel) + helper `_testRequest.ts` ; tsc + lint + vitest + Next.js build verts |
| ACTIF-04     | P2 — EN COURS      | 30 min config | **`ACTIF-04-COVERAGE-02`** validé en CI 2026-04-15 (47.97/39.6/43.33/49.19 %) — seuils **46/38/42/48** stables ; poursuivre par nouveaux tests hooks puis remesure avant bump suivant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **ACTIF-02-CHATMESSAGES-01** | `ChatMessagesView.tsx` | **Exception délibérée** : **`<img>` natif** conservé — `message.imageUrl` vient du SSE sans contrat de dimensions ni d’hôte (**HTTP(S)** arbitraire, **`blob:`**, **`data:`**) ; le layout **`max-h-64 w-full object-cover`** s’appuie sur les dimensions intrinsèques — **`next/image`** sans largeur/hauteur fictives ou **`fill`** re-dimensionnant le conteneur aurait dévié du rendu validé produit. Commentaire **`eslint-disable`** renforcé in-file ; tests co-localisés **`components/chat/ChatMessagesView.test.tsx`** (placeholder + loader, image seule, texte + image, HTTPS hors allowlist, `blob:` / `data:`, KaTeX, `role="alert"`, parité **embedded** / **drawer**). |
...
### ~~[ACTIF-03]~~ Tests non co-localisés — **FERMÉ**

**Priorité :** ~~P2~~ | **Dimension :** D5 Modularité | **`[RÉSOLU]`** Phase D commit `905419a`, 2026-04-14
...
**Avancement 2026-04-12 (lot `ACTIF-03-MICROHOOKS-COLOCATE-01`, pilote ACTIF-03)** : lot groupé **5** tests co-localisés sous **`frontend/hooks/`** sans changer la logique ni **`vitest.config.ts`** : **`useAdminContentPageController.test.ts`**, **`useChallengesStats.test.tsx`**, **`useChallengeTranslations.test.tsx`**, **`useRecommendationsReason.test.ts`** (couverture **`formatRecommendationReasonDisplay`** depuis **`useRecommendations`**), **`useAccessibleAnimation.test.ts`** (impl **`@/lib/hooks/useAccessibleAnimation`** ; test placé sous **`hooks/`** selon périmètre lot). La série hooks co-localisés s’étend.

**Avancement 2026-04-12 (lot `ACTIF-03-PROGRESS-COLOCATE-01`, pilote ACTIF-03)** : lot groupé **3** tests progression / liste co-localisés sous **`frontend/hooks/`** sans changer la logique ni **`vitest.config.ts`** : **`useCompletedItems.test.tsx`**, **`usePaginatedContent.test.tsx`**, **`useProgressTimeline.test.tsx`**. La série hooks co-localisés s’étend encore.

**Avancement 2026-04-12 (lot `ACTIF-03-USECHAT-COLOCATE-01`, pilote ACTIF-03)** : **`frontend/hooks/chat/useChat.test.tsx`** co-localisé auprès de **`useChat.ts`** (flux Chatbot partagé, mock **`streamChat`**). Lot strictement borné au déplacement ; pas de changement de logique de test ni de **`vitest.config.ts`**.

**Avancement 2026-04-12 (lot `ACTIF-03-GUESTCHATACCESS-COLOCATE-01`, pilote ACTIF-03)** : **`frontend/hooks/chat/useGuestChatAccess.test.tsx`** co-localisé auprès de **`useGuestChatAccess.ts`** (**`guestChatSession`**).

**Avancement 2026-04-12 (lot `ACTIF-03-USEPROFILE-COLOCATE-01`, pilote ACTIF-03)** : **`frontend/hooks/useProfilePageController.test.ts`** co-localisé auprès de **`useProfilePageController.ts`** (mocks **`useProfile`**, **`themeStore`**, **`useAgeGroupDisplay`**).

**Avancement 2026-04-12 (lot `ACTIF-03-CHAT-COMPONENTS-COLOCATE-01`, pilote ACTIF-03)** : lot groupé **3** tests composants chat déplacés sous **`frontend/components/chat/`** sans changer la logique ni **`vitest.config.ts`** : **`ChatMessagesView.test.tsx`** (**`ChatMessagesView.tsx`**), **`ChatbotFloating.test.tsx`** (**`ChatbotFloating.tsx`**), **`ChatbotFloatingGlobal.test.tsx`** (**`ChatbotFloatingGlobal.tsx`**) ; l’ancien dossier **`__tests__/unit/components/chat/`** est vidé pour ce périmètre.

**Avancement 2026-04-12 (lot `ACTIF-03-VISUALIZATION-COLOCATE-01`, pilote ACTIF-03)** : lot **2** tests visualisation déplacés sous **`frontend/components/challenges/visualizations/`** sans changer la logique ni **`vitest.config.ts`** : **`VisualizationRenderers.test.tsx`** (**`ChessRenderer`**, **`GraphRenderer`**), **`VisualRenderer.symmetry.test.tsx`** (helpers exportés **`VisualRenderer`**) ; suppression des copies sous **`frontend/__tests__/unit/components/`** pour ces fichiers.

**Avancement 2026-04-12 (lot `ACTIF-03-BADGES-COMPONENTS-COLOCATE-01`, pilote ACTIF-03)** : lot groupé **3** tests composants badges déplacés sous **`frontend/components/badges/`** sans changer la logique ni **`vitest.config.ts`** : **`BadgeCard.test.tsx`**, **`BadgeIcon.test.tsx`**, **`BadgesProgressTabsSection.test.tsx`** (sources **`BadgeCard`**, **`BadgeIcon`**, **`BadgesProgressTabsSection`**) ; suppression des copies sous **`frontend/__tests__/unit/components/`** et **`frontend/__tests__/unit/components/badges/`** pour ces fichiers.

**Avancement 2026-04-13 (lot `ACTIF-03-DASHBOARD-SMALL-COMPONENTS-COLOCATE-01`)** : lot groupé **4** tests petits composants dashboard co-localisés sous **`frontend/components/dashboard/`** sans changer la logique ni **`vitest.config.ts`** : **`LevelIndicator.test.tsx`**, **`Recommendations.test.tsx`**, **`SpacedRepetitionSummaryWidget.test.tsx`**, **`SpacedRepetitionSummaryWidget.reviewCta.test.tsx`** ; suppression des copies sous **`frontend/__tests__/unit/components/dashboard/`** ; **ACTIF-03** reste ouvert.

**Avancement 2026-04-13 (lot `ACTIF-03-DASHBOARD-CHARTS-COLOCATE-01`)** : lot groupé **2** tests composants dashboard « charts » co-localisés sous **`frontend/components/dashboard/`** sans changer la logique ni **`vitest.config.ts`** : **`CategoryAccuracyChart.test.tsx`** (**`CategoryAccuracyChart`**), **`DashboardCategoryRadarChart.test.tsx`** (**`DashboardCategoryRadarPlot`** depuis **`DashboardCategoryRadarChart`**) ; suppression des copies sous **`frontend/__tests__/unit/components/dashboard/`** ; **ACTIF-03** reste ouvert.

**Avancement 2026-04-13 (lot `ACTIF-03-APP-BOOTSTRAP-COLOCATE-01`)** : lot groupé **3** tests bootstrap / runtime client co-localisés à côté de leurs sources sans changer la logique ni **`vitest.config.ts`** : **`Providers.test.tsx`** (**`frontend/components/providers/`**), **`LocaleInitializer.test.tsx`** (**`frontend/components/locale/`**), **`ProtectedRoute.test.tsx`** (**`frontend/components/auth/`**, déplacé depuis **`frontend/__tests__/unit/components/ProtectedRoute.test.tsx`**) ; suppression des copies sous **`frontend/__tests__/unit/components/`** (**`providers/`**, **`locale/`**, fichier racine) ; **ACTIF-03** reste ouvert.

**Avancement 2026-04-13 (lot `ACTIF-03-LAYOUT-HEADER-COLOCATE-01`)** : **`Header.test.tsx`** co-localisé sous **`frontend/components/layout/`** auprès de **`Header.tsx`** (déplacement depuis **`frontend/__tests__/unit/components/layout/`**) ; pas de changement de logique ni de **`vitest.config.ts`** ; **ACTIF-03** reste ouvert.

**Avancement 2026-04-14 (lot `ACTIF-03-APP-API-ROUTES-COLOCATE-01`)** : lot groupé **5** tests handlers Next (`route.test.ts`) co-localisés à côté de chaque **`route.ts`** sous **`frontend/app/api/`** : **`chat`**, **`chat/stream`**, **`auth/sync-cookie`**, **`exercises/generate-ai-stream`**, **`challenges/generate-ai-stream`** ; suppression des copies sous **`frontend/__tests__/unit/app/api/`** pour ces fichiers ; imports **`POST`** depuis **`./route`** ; les deux tests SSE réutilisent **`createPostJsonRequest`** via **`@/__tests__/unit/app/api/_testRequest`** (helper historique inchangé) ; **`vitest.config.ts`** inchangé ; **ACTIF-03** reste ouvert.

La migration globale des ~130 autres fichiers **n’est pas** revendiquée ; **ACTIF-03** reste ouvert. **Terrain 2026-04-12** : la co-localisation des tests **hooks** sous **`frontend/__tests__/unit/hooks/`** est **soldée pour cet emplacement historique** (plus de fichier de test actif ; seul le sous-dossier vide `chat/` peut subsister localement) ; **terrain 2026-04-12** : **`frontend/__tests__/unit/components/chat/`** n’abrite plus les trois tests chat ci-dessus (désormais **`frontend/components/chat/*.test.tsx`**) ; le reste du périmètre **`frontend/__tests__/unit/`** (pages, routes API, etc.) reste hors scope d’un seul lot.
...
**Avancement 2026-04-14 (lot `ACTIF-04-COVERAGE-02`)** : nouvelle mesure CI (**`ubuntu-latest`**, Node 20, 2026-04-14) — agrégat **All files** :
...
Seuils remontés à **46 / 38 / 42 / 48** (règle **`floor(mesure CI %) − 1`** par axe). La progression reflète les co-localisations de tests ACTIF-03 (nombreux hooks testés). ACTIF-04 reste ouvert — objectif 55 % non encore atteint.

**Avancement 2026-04-13 (lot `QF-06B-VITEST-ALIGN-01`)** : après le merge Dependabot **`@vitest/ui`**, la résolution effective du lockfile a fait passer toute la famille **Vitest** en **4.1.4** (**`vitest`**, **`@vitest/ui`**, **`@vitest/coverage-v8`**). **`frontend/package.json`** a été réaligné sur cette réalité ; **`frontend/vitest.config.ts`** reste inchangé. Travaux utiles identifiés pour la suite : (1) profiter du correctif séquentiel **mock/unmock** quand une suite **mock-heavy** est retouchée, pour supprimer d’éventuels workarounds défensifs devenus inutiles ; (2) conserver la prochaine re-mesure **CI** comme seul déclencheur d’un bump **ACTIF-04**. Hors scope volontaire : mode **browser** Vitest, **`experimental.matchesTags`**, **`experimental.vcsProvider`**, revue de **`setupFiles`**.

**Ratio hooks recalculé terrain (2026-04-11)** : 22 hooks avec test / 56 total = **34 hooks sans test (61 %)**. Historique audit initial : 71 % (52 hooks, avant les lots de tests ci-dessous). Avancements : (1) `TEST-DIAGNOSTIC-HOOK-01` — `useDiagnostic` ; (2) `TEST-SUBMIT-ANSWER-01` — `useSubmitAnswer` ; (3) `TEST-IRT-SCORES-01` — `useIrtScores` ; (4) `TEST-AI-GENERATOR-01` — `useAIExerciseGenerator`. Hooks sans test prioritaires : `useAdminUsers`, `useBadges`, `useChallenges`, `useExercises`, `useSettings`, `useLeaderboard`. (`useAuth` : **`hooks/useAuth.test.ts`** — **`ACTIF-03-USEAUTH-COLOCATE-01`** ; `useIrtScores` : **`hooks/useIrtScores.test.ts`** — **`ACTIF-03-USEIRT-COLOCATE-01`** ; `useAIExerciseGenerator` : **`hooks/useAIExerciseGenerator.test.ts`** — **`ACTIF-03-USEAI-COLOCATE-01`** ; `useSettingsPageController` : **`hooks/useSettingsPageController.test.ts`** — **`ACTIF-03-USESETTINGS-COLOCATE-01`** ; `useBadgesPageController` : **`hooks/useBadgesPageController.test.ts`** — **`ACTIF-03-USEBADGES-COLOCATE-01`** ; `useDashboardPageController` : **`hooks/useDashboardPageController.test.ts`** — **`ACTIF-03-DASHBOARD-COLOCATE-01`** ; `useContentListPageController` : **`hooks/useContentListPageController.test.tsx`** — **`ACTIF-03-CONTENTLIST-COLOCATE-01`** ; `useChallengeSolverController` : **`hooks/useChallengeSolverController.test.tsx`** — **`ACTIF-03-CHALLENGESOLVER-COLOCATE-01`** ; `useExerciseSolverController` : **`hooks/useExerciseSolverController.test.ts`** — **`ACTIF-03-EXERCISESOLVER-COLOCATE-01`** ; lot **`ACTIF-03-MICROHOOKS-COLOCATE-01`** : **`hooks/useAdminContentPageController.test.ts`**, **`hooks/useChallengesStats.test.tsx`**, **`hooks/useChallengeTranslations.test.tsx`**, **`hooks/useRecommendationsReason.test.ts`**, **`hooks/useAccessibleAnimation.test.ts`** ; lot **`ACTIF-03-PROGRESS-COLOCATE-01`** : **`hooks/useCompletedItems.test.tsx`**, **`hooks/usePaginatedContent.test.tsx`**, **`hooks/useProgressTimeline.test.tsx`** ; `useChat` : **`hooks/chat/useChat.test.tsx`** — **`ACTIF-03-USECHAT-COLOCATE-01`** ; `useGuestChatAccess` : **`hooks/chat/useGuestChatAccess.test.tsx`** — **`ACTIF-03-GUESTCHATACCESS-COLOCATE-01`** ; `useProfilePageController` : **`hooks/useProfilePageController.test.ts`** — **`ACTIF-03-USEPROFILE-COLOCATE-01`**.)
...
| Fichier                                | Lignes | Note                                       |
| -------------------------------------- | ------ | ------------------------------------------ |
| `hooks/useProfilePageController.ts`    | 396    | Aucun bug connu                            |
...
| Coverage-gate          | Seuils vitest : historique **QF-06** (43/36/39/44 puis 39/33/37/40 %) → **`ACTIF-04-COVERAGE-01`** → **43/36/40/44** (CI 2026-04-12 : 44.57/37.22/41.47/45.68 %) → **`ACTIF-04-COVERAGE-02`** → **46/38/42/48** (CI 2026-04-14 : 47.96/39.6/43.29/49.18 %, règle **floor−1**) | QF-06 + ACTIF-04-COVERAGE-01 + ACTIF-04-COVERAGE-02 | 2026-04-14         |
...
`[DÉCISION]` **`ChatMessagesView.tsx`** (lot **ACTIF-02-CHATMESSAGES-01**) : **`<img>` natif** par **exception produit délibérée** — pas seulement « en attendant » : toute bascule **`next/image`** exigerait contrat URL/dimensions ou **`fill`** avec conteneur dimensionné, ce qui **n’est pas** acceptable sans changement UX (voir commentaire in-file et tests **`frontend/components/chat/ChatMessagesView.test.tsx`**). **`resolveNextImageRemoteDelivery`** ne s’applique pas à ce flux.
...
- Post-upgrade **Vitest 4.1.x** : quand une suite **mock-heavy** est rouverte, vérifier si les workarounds défensifs liés aux enchaînements **mock/unmock** peuvent être supprimés ; ne rien changer « en masse » sans besoin local — 15 à 30 min par suite concernée
- ~~Prochain bump **ACTIF-04**~~ : **`ACTIF-04-COVERAGE-02`** effectué (2026-04-14) — seuils **46/38/42/48** depuis mesure CI 47.96/39.6/43.29/49.18 % — **validé en CI 2026-04-15** (47.97/39.6/43.33/49.19 %, marges +1.97/+1.6/+1.33/+1.19) ; prochain bump après nouveaux lots de tests et remesure CI
- ~~`console.error` sans garde dev~~ : **fait** (commit `1aa46b8`, 2026-04-15) — `useAcademyStats.ts:88`, `useSettings.ts:140`, `useSettingsPageController.ts:166` (bonus), `auth-session-sync.ts:111` → `debugError()` ; tsc + lint verts
- Décomposition `useProfilePageController.ts` (396 L) — non bloquant
...
_Audit initial : 2026-04-09. Dernière mise à jour : 2026-04-14. Dernière vérification terrain : 2026-04-14 (**ACTIF-03-APP-API-ROUTES-COLOCATE-01** : cinq `app/api/**/route.test.ts` co-localisés). Dernière vérification terrain détaillée : 2026-04-13 (**ACTIF-02-CHATMESSAGES-01** : `ChatMessagesView` exception `<img>` + tests ; **ACTIF-03-USEAUTH-COLOCATE-01** : `hooks/useAuth.test.ts` co-localisé ; **ACTIF-03-BUILDCSP-COLOCATE-01** / **ACTIF-03-MIDDLEWARECSP-COLOCATE-01** : `lib/security/buildContentSecurityPolicy.test.ts` + `lib/security/middlewareCsp.test.ts` co-localisés ; **ACTIF-03-USEIRT-COLOCATE-01** : `hooks/useIrtScores.test.ts` co-localisé ; **ACTIF-03-USEAI-COLOCATE-01** : `hooks/useAIExerciseGenerator.test.ts` co-localisé ; **ACTIF-03-USESETTINGS-COLOCATE-01** : `hooks/useSettingsPageController.test.ts` co-localisé ; **ACTIF-03-USEBADGES-COLOCATE-01** : `hooks/useBadgesPageController.test.ts` co-localisé ; **ACTIF-03-DASHBOARD-COLOCATE-01** : `hooks/useDashboardPageController.test.ts` co-localisé ; **ACTIF-03-CONTENTLIST-COLOCATE-01** : `hooks/useContentListPageController.test.tsx` co-localisé ; **ACTIF-03-CHALLENGESOLVER-COLOCATE-01** : `hooks/useChallengeSolverController.test.tsx` co-localisé ; **ACTIF-03-EXERCISESOLVER-COLOCATE-01** : `hooks/useExerciseSolverController.test.ts` co-localisé ; **ACTIF-03-MICROHOOKS-COLOCATE-01** : 5 tests hooks co-localisés sous `hooks/` ; **ACTIF-03-PROGRESS-COLOCATE-01** : 3 tests progress/list co-localisés sous `hooks/` ; **ACTIF-03-USECHAT-COLOCATE-01** : `hooks/chat/useChat.test.tsx` co-localisé ; **ACTIF-03-GUESTCHATACCESS-COLOCATE-01** : `hooks/chat/useGuestChatAccess.test.tsx` co-localisé ; **ACTIF-03-USEPROFILE-COLOCATE-01** : `hooks/useProfilePageController.test.ts` co-localisé ; **ACTIF-04-COVERAGE-01** : seuils vitest **43/36/40/44** depuis baseline CI GitHub Actions **44.57/37.22/41.47/45.68 %** ; **QF-06B-VITEST-ALIGN-01** : famille **Vitest 4.1.4** réalignée dans `package.json` et backlog restreint documenté ; **ACTIF-06-ADMIN-USERS-01** : `useAdminUsersPageController` + page users allégée ; **ACTIF-06-AI-MONITORING-01** : `useAdminAiMonitoringPageController` + page ai-monitoring allégée ; **finding ACTIF-06 fermé** ; **finding ACTIF-02 fermé**). Toutes les assertions citent fichier:ligne lu directement._
