...
> **Version :** 2026-04-16 (audit initial 2026-04-09, mis à jour au fil des lots puis réalignement documentaire vérité terrain)
...
Le lot `ACTIF-03-APP-API-ROUTES-COLOCATE-01` fermait encore un sous-scope ; la fermeture globale de **ACTIF-03** est désormais revendiquée au **2026-04-14** : `frontend/__tests__/unit/` est réduit au seul garde-fou **`architecture/frontendGuardrails.test.ts`** et au helper conservé **`app/api/_testRequest.ts`**. **Terrain 2026-04-16** : la co-localisation des tests **hooks** sous **`frontend/__tests__/unit/hooks/`** est soldée pour cet emplacement historique, les tests chat vivent sous **`frontend/components/chat/*.test.tsx`**, et les handlers Next testés vivent sous **`frontend/app/api/**/route.test.ts`**.
...
Les seuils intermédiaires **43 / 36 / 40 / 44** ont servi de palier de transition après la première mesure CI. **État vérité terrain 2026-04-16** : `vitest.config.ts` est réglé sur **46 / 38 / 42 / 48** ; **ACTIF-04** reste ouvert car l’horizon « couverture large » (ex. 55 %) n’est pas atteint et tout nouveau bump exige une **nouvelle mesure CI** après écriture de tests.
...
_Audit initial : 2026-04-09. Dernière mise à jour : 2026-04-16. Dernière vérification terrain : 2026-04-16 (relecture documentaire exhaustive + vérification croisée des chemins actifs, seuils Vitest **46/38/42/48**, fermeture ACTIF-03, maintien ACTIF-04, archives et index réalignés). Dernière vérification terrain détaillée : 2026-04-15 (**ACTIF-04-COVERAGE-02** validé en CI, famille Vitest **4.1.4**, route handlers co-localisés sous `app/api/**/route.test.ts`, `frontend/__tests__/unit/` réduit au garde-fou architecture + helper `_testRequest.ts`). Toutes les assertions citent fichier:ligne lu directement._
