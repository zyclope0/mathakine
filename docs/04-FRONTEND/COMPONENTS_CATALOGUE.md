...
**Contrats et logique de présentation partagée (FFI-L20D)** : `lib/badges/types.ts` (progression, rareté, tri) ; `lib/badges/badgePresentation.ts` (médailles SVG, difficulté/glow, tri grille, motivation verrouillée). `BadgeCard` / `BadgeGrid` / `BadgesProgressTabsSection` consomment ces helpers sans dupliquer les branches.
