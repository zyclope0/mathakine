...
def test_render_connection():
    ...
    if not database_url:
        ...
        assert False, "La variable d'environnement DATABASE_URL n'est pas définie"
    ...
    try:
        ...
        conn.close()
        assert True, "Connexion à PostgreSQL réussie"

    except Exception as e:
        logger.error(f"Erreur de connexion à PostgreSQL: {e}")
        assert False, f"Erreur de connexion à PostgreSQL: {e}"
