...
async def test_complete_user_deletion_cascade(db_session):
    ...
    assert (
        ...
        .filter(LogicChallengeAttempt.id == challenge_attempt_id)
        .count()
        == 1
    )
...
async def test_multi_level_cascade(db_session):
    ...
    assert (
        ...
        .filter(Exercise.id.in_([exercise1_id, exercise2_id]))
        .count()
        == 2
    )
    assert (
        db_session.query(Attempt)
        .filter(Attempt.id.in_([attempt1a_id, attempt1b_id, attempt2_id]))
        .count()
        == 3
    )
    assert (
        db_session.query(Recommendation)
        .filter(Recommendation.id == recommendation1_id)
        .count()
        == 1
    )
