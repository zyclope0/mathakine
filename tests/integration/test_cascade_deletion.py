...
def setup_exercise_with_attempts(db_session):
    ...
    attempt1 = Attempt(
        user_id=user.id, exercise_id=exercise.id, user_answer="4", is_correct=True
    )
    attempt2 = Attempt(
        user_id=user.id, exercise_id=exercise.id, user_answer="5", is_correct=False
    )
...
async def test_exercise_cascade_deletion(db_session, setup_exercise_with_attempts):
    ...
    attempts = (
        db_session.query(Attempt).filter(Attempt.exercise_id == exercise_id).all()
    )
...
def setup_logic_challenge_with_attempts(db_session):
    ...
    attempt1 = LogicChallengeAttempt(
        user_id=user.id, challenge_id=challenge.id, user_solution="42", is_correct=True
    )
    attempt2 = LogicChallengeAttempt(
        user_id=user.id, challenge_id=challenge.id, user_solution="41", is_correct=False
    )
    db_session.add(attempt1)
    db_session.add(attempt2)
    db_session.commit()
...
async def test_logic_challenge_cascade_deletion(
    db_session, setup_logic_challenge_with_attempts
):
    ...
    for attempt_id in attempt_ids:
        attempt = (
            db_session.query(LogicChallengeAttempt)
            .filter(LogicChallengeAttempt.id == attempt_id)
            .first()
        )
        assert attempt is None
...
async def test_user_cascade_deletion(db_session):
    ...
    ex_attempt1 = Attempt(
        user_id=user.id, exercise_id=exercise.id, user_answer="10", is_correct=True
    )
    ex_attempt2 = Attempt(
        user_id=user.id, exercise_id=exercise.id, user_answer="9", is_correct=False
    )
    ...
    recommendations = (
        db_session.query(Recommendation).filter(Recommendation.user_id == user_id).all()
    )
...
async def test_user_recommendations_cascade_deletion(
    db_session, setup_user_with_recommendations
):
...
async def test_exercise_recommendations_no_cascade(
    db_session, setup_user_with_recommendations
):
    ...
    recommendations_with_exercise = (
        db_session.query(Recommendation)
        .filter(Recommendation.exercise_id == exercise_id)
        .all()
    )
