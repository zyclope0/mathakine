...
def test_complete_exercise_workflow(db_session):
    ...
    all_exercises = db_session.query(Exercise).filter(
    ...
    for ex in all_exercises[-3:]:  # Afficher les 3 derniers exercices
        print(f"  - {ex.title}: {ex.exercise_type}/{ex.difficulty} (active: {ex.is_active}, archived: {ex.is_archived})")
    
    initial_recommendations = RecommendationService.generate_recommendations(db_session, student.id)
    ...
    updated_recommendations = RecommendationService.generate_recommendations(db_session, student.id)
    
    # Vérifier que des recommandations ont été générées
    assert len(updated_recommendations) > 0, "Des recommandations mises à jour devraient être générées"
    ...
    if updated_recommendations:
        RecommendationService.mark_recommendation_as_completed(db_session, updated_recommendations[0].id)
        
        # Vérifier que la recommandation est marquée comme complétée
        completed_rec = db_session.query(Recommendation).filter(
            Recommendation.id == updated_recommendations[0].id
        ).first()
        
        assert completed_rec.is_completed is True, "La recommandation devrait être marquée comme complétée"
        assert completed_rec.completed_at is not None, "La date de complétion devrait être définie"
...
def test_exercise_statistics_and_trends(db_session):
    ...
    """
    pytest.skip("Test temporairement désactivé - dépend de fonctionnalités non implémentées")
    ...
    for ex_type in [get_enum_value(ExerciseType, ExerciseType.ADDITION.value, db_session), get_enum_value(ExerciseType, ExerciseType.SOUSTRACTION.value, db_session), get_enum_value(ExerciseType, ExerciseType.MULTIPLICATION.value, db_session), get_enum_value(ExerciseType, ExerciseType.DIVISION.value, db_session)]:
    ...
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.CHEVALIER.value, db_session)]:
        exercise = exercises[get_enum_value(ExerciseType, ExerciseType.ADDITION.value, db_session)][difficulty.value]
        for i in range(10):
            attempt = Attempt(
                user_id=user.id,
    ...
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session)]:
        exercise = exercises[get_enum_value(ExerciseType, ExerciseType.SOUSTRACTION.value, db_session)][difficulty.value]
        for i in range(10):
            attempt = Attempt(
                user_id=user.id,
                exercise_id=exercise.id,
    ...
    exercise = exercises[get_enum_value(ExerciseType, ExerciseType.SOUSTRACTION.value, db_session)][get_enum_value(DifficultyLevel, DifficultyLevel.CHEVALIER.value, db_session)]
    for i in range(5):
        attempt = Attempt(
            user_id=user.id,
            exercise_id=exercise.id,
    ...
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session)]:
        exercise = exercises[get_enum_value(ExerciseType, ExerciseType.MULTIPLICATION.value, db_session)][difficulty.value]
        for i in range(10):
            # L'utilisateur s'améliore avec le temps (les dernières tentatives sont plus souvent correctes)
            is_correct = i >= 5  # Les 5 premières tentatives échouent, les 5 dernières réussissent
            attempt = Attempt(
                user_id=user.id,
                exercise_id=exercise.id,
    ...
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session)]:
        exercise = exercises[get_enum_value(ExerciseType, ExerciseType.DIVISION.value, db_session)][difficulty.value]
        for i in range(5):
            attempt = Attempt(
                user_id=user.id,
                exercise_id=exercise.id,
    ...
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.CHEVALIER.value, db_session)]:
        progress = db_session.query(Progress).filter(
            ...
            Progress.exercise_type == get_enum_value(ExerciseType, ExerciseType.ADDITION.value, db_session),
            Progress.difficulty == difficulty
        ).first()
        
        assert progress is not None, f"Le progrès en addition niveau {difficulty} devrait exister"
        assert progress.total_attempts == 10, f"Il devrait y avoir 10 tentatives en addition niveau {difficulty}"
        assert progress.correct_attempts == 10, f"Toutes les tentatives en addition niveau {difficulty} devraient être correctes"
        
        # Le taux de réussite devrait être de 100%
        success_rate = progress.correct_attempts / progress.total_attempts
        assert success_rate == 1.0, f"Le taux de réussite en addition niveau {difficulty} devrait être de 100%"
    
    # Progrès en division - devrait être mauvais
    for difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.PADAWAN.value, db_session)]:
        progress = db_session.query(Progress).filter(
            Progress.user_id == user.id,
            Progress.exercise_type == get_enum_value(ExerciseType, ExerciseType.DIVISION.value, db_session),
            Progress.difficulty == difficulty
        ).first()
        
        assert progress is not None, f"Le progrès en division niveau {difficulty} devrait exister"
        assert progress.total_attempts == 5, f"Il devrait y avoir 5 tentatives en division niveau {difficulty}"
        assert progress.correct_attempts == 0, f"Aucune tentative en division niveau {difficulty} ne devrait être correcte"
        
        # Le taux de réussite devrait être de 0%
        success_rate = progress.correct_attempts / progress.total_attempts if progress.total_attempts > 0 else 0
        assert success_rate == 0.0, f"Le taux de réussite en division niveau {difficulty} devrait être de 0%"
    ...
    assert ExerciseType.ADDITION.value in rec_by_type_level, "Il devrait y avoir des recommandations d'addition"
    addition_recs = rec_by_type_level[get_enum_value(ExerciseType, ExerciseType.ADDITION.value, db_session)]
    ...
    assert master_level_exists or chevalier_level_exists, "Il devrait y avoir des recommandations d'addition de niveau Maître ou Chevalier"
    
    # b) Soustraction: l'utilisateur devrait recevoir des recommandations pour s'améliorer au niveau Chevalier
    assert ExerciseType.SOUSTRACTION.value in rec_by_type_level, "Il devrait y avoir des recommandations de soustraction"
    soustraction_recs = rec_by_type_level[get_enum_value(ExerciseType, ExerciseType.SOUSTRACTION.value, db_session)]
    assert DifficultyLevel.CHEVALIER.value in soustraction_recs, "Il devrait y avoir des recommandations de soustraction niveau Chevalier"
    
    # c) Division: l'utilisateur devrait recevoir des recommandations de niveau Initié pour renforcer ses bases
    assert ExerciseType.DIVISION.value in rec_by_type_level, "Il devrait y avoir des recommandations de division"
    division_recs = rec_by_type_level[get_enum_value(ExerciseType, ExerciseType.DIVISION.value, db_session)]
    assert DifficultyLevel.INITIE.value in division_recs, "Il devrait y avoir des recommandations de division niveau Initié"
    ...
    for rec in recommendations:
        if rec.exercise_type == get_enum_value(ExerciseType, ExerciseType.ADDITION.value, db_session) and rec.difficulty in [get_enum_value(DifficultyLevel, DifficultyLevel.MAITRE.value, db_session), get_enum_value(DifficultyLevel, DifficultyLevel.CHEVALIER.value, db_session)]:
            assert "niveau" in rec.reason.lower() or "maîtrise" in rec.reason.lower(), "La raison devrait mentionner la progression vers un niveau supérieur"
        
        if rec.exercise_type == get_enum_value(ExerciseType, ExerciseType.DIVISION.value, db_session) and rec.difficulty == get_enum_value(DifficultyLevel, DifficultyLevel.INITIE.value, db_session):
            assert "renforcer" in rec.reason.lower() or "difficulté" in rec.reason.lower(), "La raison devrait mentionner le besoin de renforcement" 