...
async def test_user_exercise_flow(client):
    ...
    try:
        ...
        ), f"Erreur lors de la création de l'utilisateur: {response.text}"
        verify_user_email_for_tests(user_data["username"])
        user_response = response.json()
        assert (
            "id" in user_response
        ), "L'ID de l'utilisateur est manquant dans la réponse"
        ...
        ), "Le token d'accès est manquant dans la réponse"

        # 3. Création d'un exercice via POST /api/exercises/generate (endpoint disponible)
        headers = {"Authorization": f"Bearer {token_response['access_token']}"}
        generate_data = {"exercise_type": "addition", "age_group": "6-8", "save": True}
        response = await client.post(
            "/api/exercises/generate", json=generate_data, headers=headers
        )
        assert response.status_code in [
            200,
            201,
        ], f"Erreur lors de la génération de l'exercice: {response.text}"
        exercise_response = response.json()
        assert (
            "id" in exercise_response
        ), "L'ID de l'exercice est manquant dans la réponse"
        ...
        response = await client.post(
            f"/api/exercises/{exercise_id}/attempt", json=attempt_data, headers=headers
        )
        assert (
            response.status_code == 200
        ), f"Erreur lors de la soumission de la tentative: {response.text}"
        attempt_response = response.json()
        assert (
            attempt_response["is_correct"] is True
        ), "La tentative devrait être correcte"
        ...
        assert (
            response.status_code == 200
        ), f"Erreur lors de la récupération des statistiques: {response.text}"
        ...
        total_key = next(
            (
                ...
                if k in stats_response
            ),
            None,
        )
        assert (
            total_key
        ), f"Les statistiques de tentatives sont manquantes: {list(stats_response.keys())}"
        assert (
            stats_response[total_key] >= 1
        ), "Le nombre de tentatives devrait être d'au moins 1"
...
async def test_invalid_exercise_attempt(client):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {token_response['access_token']}"}
        attempt_data = {"answer": "42", "time_spent": 30}
        response = await client.post(
            "/api/exercises/99999/attempt", json=attempt_data, headers=headers
        )
        assert (
            response.status_code == 404
        ), f"Une tentative sur un exercice inexistant devrait retourner 404, mais a retourné {response.status_code}: {response.text}"
        ...
        bad_attempt_data = {"answer": wrong_answer, "time_spent": 10}
        response = await client.post(
            f"/api/exercises/{exercise_id}/attempt",
            json=bad_attempt_data,
            headers=headers,
        )
        assert (
            response.status_code == 200
        ), f"Erreur lors de la soumission de la tentative incorrecte: {response.text}"
        result = response.json()
        assert (
            result["is_correct"] is False
        ), "La tentative avec une réponse incorrecte devrait être marquée comme incorrecte"
