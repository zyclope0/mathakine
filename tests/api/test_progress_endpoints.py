...
async def test_get_user_progress_by_type(padawan_client, db_session, mock_exercise):
    ...
    assert (
        exercise_type_val in data["by_category"]
    ), f"by_category devrait contenir {exercise_type_val}: {data['by_category']}"
...
async def test_register_exercise_attempt(padawan_client, db_session, mock_exercise):
    ...
    assert (
        db_exercise is not None
    ), f"L'exercice avec ID {exercise.id} n'existe pas dans la base de données après création"
    print(
        f"Exercice récupéré depuis la base de données: ID={db_exercise.id}, titre={db_exercise.title}"
    )
    ...
    response = await client.post(
        f"/api/exercises/{exercise.id}/attempt", json=attempt_data
    )
    ...
    if response.status_code != 200:
        ...
        check_exercise = (
            db_session.query(Exercise).filter(Exercise.id == exercise.id).first()
        )
        if check_exercise:
            print(
                f"L'exercice est toujours dans la base: ID={check_exercise.id}, is_archived={check_exercise.is_archived}"
            )
        else:
    ...
    attempts = (
        ...
        .filter(Attempt.user_id == user_id, Attempt.exercise_id == exercise.id)
        .all()
    )
    ...
    progress = (
        ...
        .filter(
            Progress.user_id == user_id, Progress.exercise_type == exercise_type_val
        )
        .first()
    )
...
async def test_register_exercise_attempt_incorrect(
    ...
    attempt_data = {"answer": incorrect_answer, "time_spent": 30}  # secondes
