...
def test_get_user_recommendations(padawan_client, db_session, mock_exercise):
    ...
    response = client.get("/api/users/me/recommendations")
...
def test_mark_recommendation_as_clicked(padawan_client, db_session, mock_exercise):
    ...
    response = client.post(f"/api/recommendations/{recommendation.id}/clicked")
...
def test_mark_recommendation_as_completed(padawan_client, db_session, mock_exercise):
    ...
    response = client.post(f"/api/recommendations/{recommendation.id}/completed")
...
def test_mark_recommendation_nonexistent(padawan_client):
    ...
    response = client.post("/api/recommendations/9999/completed")
    
    # Vérifier que la requête échoue
    assert response.status_code in [404, 422]  # 404 si la recommandation n'existe pas ou endpoint incorrect

def test_mark_other_user_recommendation(padawan_client, maitre_client, db_session, mock_exercise):
    ...
    response = maitre_client_api.post(f"/api/recommendations/{recommendation.id}/clicked")
