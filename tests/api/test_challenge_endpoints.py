...
async def test_get_logic_challenges(client):
...
async def test_get_logic_challenge_by_id(client):
    """Test de l'endpoint pour récupérer un défi logique par ID."""
    # Tester avec un ID connu (1)
    response = await client.get("/api/challenges/1")
    assert response.status_code == 200
...
async def test_get_nonexistent_challenge(client):
...
async def test_filter_challenges_by_type(client):
