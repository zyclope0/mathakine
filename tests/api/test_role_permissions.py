...
async def test_padawan_permissions(padawan_client):
    ...
    ), "Un padawan devrait pouvoir accéder aux exercices"

    # Accès autorisé: soumettre une tentative pour un exercice existant
    attempt_data = {"selected_answer": "4", "time_spent": 10.0}
    ...
    assert response.status_code in [
        200,
        404,
    ], f"Un padawan devrait pouvoir soumettre une tentative (ou recevoir 404 si l'exercice n'existe pas)"
...
async def test_maitre_permissions(maitre_client, padawan_client):
    ...
    if exercise_id is not None:
        ...
        assert response.status_code in [
            ...
            404,
            405,
        ], "Un maître devrait pouvoir modifier ou route inexistante"
...
async def test_gardien_permissions(gardien_client):
    ...
    assert (
        response.status_code == 200
    ), "Un gardien devrait pouvoir accéder aux exercices archivés"
