...
def test_create_test_users_integration(db_session):
    ...
    with patch("app.services.db_init_service.User") as MockUser:
        ...
        with (
            ...
            maitre_users = [
                u
                for u in called_users
                if hasattr(u, "role") and u.role == UserRole.MAITRE
            ]
            assert (
                len(maitre_users) > 0
            ), "Au moins un utilisateur Maître devrait être créé"
...
def test_create_test_logic_challenges_integration(db_session):
    ...
    with patch("app.services.db_init_service.LogicChallenge") as MockLogicChallenge:
        ...
        with (
            ...
            assert (
                len(called_challenges) >= 1
            ), "Au moins un défi logique devrait être créé"
...
def test_create_test_users_already_exist(db_session):
    ...
    try:
        ...
        )  # Peut retourner True ou None selon l'implémentation
    ...
    existing_user = (
        ...
        .filter_by(email=f"existing_{unique_id}@example.com")
        .first()
    )
