...
def dict_to_user(user_data):
    ...
    with Session(engine) as session:
        ...
        if preferred_difficulty:
            preferred_difficulty = adapt_enum_for_db(
                "DifficultyLevel", preferred_difficulty, session
            )
...
def test_refresh_access_token_wrong_type(db_session, mock_user):
    ...
    access_token = jwt.encode(
        {"sub": user.username, "exp": datetime.now() + token_expires, **token_data},
        settings.SECRET_KEY,
        algorithm="HS256",
    )
...
def test_refresh_access_token_tampered_token(db_session, mock_user):
    ...
    valid_token = jwt.encode(
        valid_token_data, settings.SECRET_KEY, algorithm=settings.ALGORITHM
    )
...
def test_update_user_password(db_session, mock_user):
    ...
    user.hashed_password = (
        hashed_password  # S'assurer que le mot de passe est bien défini
    )
