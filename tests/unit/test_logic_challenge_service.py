...
def test_list_challenges(db_session):
    ...
    challenges = [
        ...
        LogicChallenge(
            ...
            challenge_type=get_enum_value(
                LogicChallengeType, LogicChallengeType.PATTERN.value, db_session
            ),
        ...
        LogicChallenge(
            ...
            challenge_type=get_enum_value(
                LogicChallengeType, LogicChallengeType.PUZZLE.value, db_session
            ),
            age_group=get_enum_value(AgeGroup, AgeGroup.GROUP_13_15.value, db_session),
    ...
    non_archived_titles = [
        f"Test Séquence 9-12 {timestamp}",
        f"Test Pattern 12-13 {timestamp}",
        f"Test Puzzle 13+ {timestamp}",
    ]
    ...
    sequence_challenges = LogicChallengeService.list_challenges(
        db_session, challenge_type="SEQUENCE"
    )
    test_sequence_challenges = [
        ch for ch in sequence_challenges if timestamp in ch.title
    ]
    ...
    age_13_15_challenges = LogicChallengeService.list_challenges(
        db_session, age_group="GROUP_13_15"
    )
    test_age_challenges = [ch for ch in age_13_15_challenges if timestamp in ch.title]
    assert (
        len(test_age_challenges) == 2
    )  # Il y a 2 défis pour ce groupe d'âge (Pattern et Puzzle)
    ...
    filtered_challenges = LogicChallengeService.list_challenges(
        db_session, challenge_type="PUZZLE", age_group="GROUP_13_15"
    )
    test_filtered_challenges = [
        ch for ch in filtered_challenges if timestamp in ch.title
    ]
...
def test_list_challenges_with_empty_db(db_session):
    ...
    with patch(
        ...
        challenges_with_type = LogicChallengeService.list_challenges(
            db_session, challenge_type="PUZZLE"
        )
        challenges_with_age = LogicChallengeService.list_challenges(
            db_session, age_group="GROUP_10_12"
        )
        challenges_with_limit = LogicChallengeService.list_challenges(
            db_session, limit=10
        )
...
def test_create_challenge(db_session):
    ...
    assert str(challenge.challenge_type) in [
        "PUZZLE",
        "puzzle",
        "LogicChallengeType.PUZZLE",
    ]
    assert str(challenge.age_group) in [
        "GROUP_13_15",
        "group_13_15",
...
def test_update_challenge(db_session):
    ...
    result = LogicChallengeService.update_challenge(
        db_session, challenge.id, update_data
    )

    # Vérifications
    assert (
        result is True
    )  # La méthode update_challenge retourne un booléen, pas l'objet mis à jour
...
def test_update_nonexistent_challenge(db_session):
    ...
    result = LogicChallengeService.update_challenge(
        db_session, nonexistent_id, {"title": "New Title"}
    )
...
def test_update_challenge_with_error_handling(db_session):
    ...
    result = LogicChallengeService.update_challenge(
        db_session, challenge.id, invalid_data
    )
...
def test_delete_challenge(db_session):
    ...
    challenge = LogicChallenge(
        ...
        challenge_type=get_enum_value(
            LogicChallengeType, LogicChallengeType.VISUAL.value, db_session
        ),
        age_group=get_enum_value(AgeGroup, AgeGroup.GROUP_13_15.value, db_session),
...
def test_get_challenge_attempts(db_session):
    ...
    challenge_attempts = LogicChallengeService.get_challenge_attempts(
        db_session, challenge.id
    )
...
def test_record_attempt_with_complete_user_journey(db_session):
    ...
    assert attempt.challenge_id == challenge.id
    assert (
        attempt.user_solution == "Je pense que c'est 42"
    )  # user_solution et non user_answer
    assert attempt.is_correct is True
    assert attempt.time_spent == 120.5
    ...
    second_attempt = LogicChallengeService.record_attempt(
        db_session, second_attempt_data
    )
...
def test_update_challenge_with_complex_data(db_session):
    ...
    result = LogicChallengeService.update_challenge(
        db_session, challenge.id, updated_data
    )
    ...
    assert (
        updated_challenge.solution_explanation == "Nouvelle explication de la solution"
    )
...
def test_list_challenges_with_mock(mock_db_adapter, mock_adapt_enum):
    ...
    )  # On vérifie juste que filter a été appelé, pas avec quels arguments
    ...
    mock_type_filtered_query.all.return_value = [
        mock_challenges[0]
    ]  # Seulement le premier défi

    result_with_type = LogicChallengeService.list_challenges(
        mock_session, challenge_type="SEQUENCE"
    )
    ...
    mock_age_filtered_query.all.return_value = (
        []
    )  # Aucun résultat avec les deux filtres

    result_with_multiple_filters = LogicChallengeService.list_challenges(
        mock_session, challenge_type="SEQUENCE", age_group="GROUP_13_15"
    )
