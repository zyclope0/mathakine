...
def test_create_table_queries_contain_required_fields():
    ...
    required_fields = {
        'exercises': ['id', 'title', 'exercise_type', 'difficulty', 'question', 'correct_answer'],
    ...
    for query in create_table_queries:
        ...
        for field in required_fields[table_name]:
            assert field in query, f"Le champ '{field}' devrait être présent dans la requête CREATE TABLE de {table_name}" 