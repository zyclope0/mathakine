...
def test_get_by_field_success():
    ...
    # Appeler la méthode
    result = DatabaseAdapter.get_by_field(
        mock_session, mock_model, "field_name", "value"
    )
...
def test_list_active_success():
    ...
    with patch("app.db.adapter.hasattr", return_value=True):
        # Appeler la méthode
        result = DatabaseAdapter.list_active(
            mock_session, mock_model, limit=10, offset=0
        )
...
def test_create_success():
    ...
    with patch(
        "app.db.adapter.TransactionManager.transaction", return_value=mock_transaction
    ):
        # Appeler la méthode
...
def test_update_success():
    ...
    with (
        patch("app.db.adapter.hasattr", return_value=True),
        patch("app.db.adapter.getattr", return_value=None),
        patch("app.db.adapter.setattr") as mock_setattr,
        patch(
            "app.db.adapter.TransactionManager.transaction",
