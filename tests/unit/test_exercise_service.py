...
def test_create_exercise(db_session):
    ...
    exercise_data = {
        ...
        "difficulty": get_enum_value(
            DifficultyLevel, DifficultyLevel.MAITRE.value, db_session
        ),
        "age_group": "15-17",
        "question": "10/2=?",
        "correct_answer": "5",
...
def test_update_nonexistent_exercise(db_session):
    ...
    result = ExerciseService.update_exercise(
...
def test_delete_exercise_cascade(db_session):
    ...
    remaining_attempts = (
        db_session.query(Attempt).filter_by(exercise_id=exercise.id).count()
    )
...
def test_get_exercise_attempts(db_session):
    ...
    attempts = [
        Attempt(
            user_id=user.id, exercise_id=exercise.id, user_answer="3", is_correct=False
        ),
        Attempt(
            user_id=user.id, exercise_id=exercise.id, user_answer="4", is_correct=False
        ),
        Attempt(
            user_id=user.id, exercise_id=exercise.id, user_answer="5", is_correct=True
        ),
    ]
...
def test_list_exercises_with_mock(mock_db_adapter, mock_adapt_enum):
    ...
    result_by_type = ExerciseService.list_exercises(
        mock_session, exercise_type="ADDITION"
    )
