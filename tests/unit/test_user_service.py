...
def test_get_user_by_email():
    ...
    with patch(
        ...
        mock_get_by_field.assert_called_once_with(
            mock_session, User, "email", test_email
        )
...
def test_update_user(db_session):
    ...
    initial_role = get_enum_value(
        UserRole, UserRole.PADAWAN.value, db_session
    )  # "PADAWAN"
    user = User(
    ...
    maitre_role = get_enum_value(
        UserRole, UserRole.MAITRE.value, db_session
    )  # "MAITRE"
    update_data = {
...
def test_update_nonexistent_user(db_session):
    ...
    result = UserService.update_user(
        db_session, nonexistent_id, {"full_name": "New Name"}
    )
...
def test_get_user_stats(db_session):
    ...
    with patch(
        ...
        mock_stats = {
            ...
            "success_rate": 67,
            "by_exercise_type": {
                "ADDITION": {"total": 2, "correct": 2, "success_rate": 100},
                "SOUSTRACTION": {"total": 1, "correct": 0, "success_rate": 0},
            },
        }
...
def test_get_user_stats_with_exception():
    ...
    with patch(
        ...
        assert (
            result["stats_error"] == "Erreur lors de la récupération des statistiques"
        )
...
def test_get_user_stats_with_multiple_exercise_types(db_session):
    ...
    with patch(
        ...
        mock_stats = {
            ...
            "by_exercise_type": {
                "ADDITION": {"total": 2, "correct": 2, "success_rate": 100},
                "SOUSTRACTION": {"total": 2, "correct": 1, "success_rate": 50},
                "MULTIPLICATION": {"total": 1, "correct": 0, "success_rate": 0},
            },
        }
        mock_get_stats.return_value = mock_stats
    ...
    with patch(
        ...
        with (
            ...
            mock_create.assert_called_once_with(
                mock_transaction_result, User, user_data
            )
...
def test_cascade_delete_user_with_relationships(db_session):
    ...
    assert (
        ...
        .filter_by(id=challenge_attempt_id)
        .first()
        is None
    )
...
def test_get_user_stats_with_specific_exercise_type(db_session):
    ...
    with patch(
        ...
        mock_stats = {
            ...
            "success_rate": 60,
            "by_exercise_type": {
                "addition": {"total": 5, "correct": 3, "success_rate": 60}
            },
        }
        mock_get_stats.return_value = mock_stats
...
def test_get_user_stats_performance_by_difficulty(db_session):
    ...
    with patch(
        ...
        mock_stats = {
            ...
            "average_time": 45.5,
            "by_exercise_type": {
                ExerciseType.ADDITION: {"total": 6, "correct": 3, "success_rate": 50}
            },
        }
        mock_get_stats.return_value = mock_stats
...
def test_get_user_stats_with_complex_relations(db_session):
    ...
    with patch(
        ...
        mock_stats = {
            ...
            "average_time": 35.2,
            "by_exercise_type": {
                ExerciseType.ADDITION: {"total": 5, "correct": 3, "success_rate": 60},
...
def test_get_user_stats_with_malformed_data(db_session):
    ...
    exercise = Exercise(
        ...
        difficulty=get_enum_value(
            DifficultyLevel, DifficultyLevel.INITIE, db_session
        ),  # difficulté standard
        age_group="6-8",
        question="Question?",
        correct_answer="42",
...
def test_user_roles_adaptation_for_different_databases():
    ...
    assert (
        adapt_enum_for_db("UserRole", padawan_value, mock_sqlite_session)
        == ENUM_MAPPING[("UserRole", padawan_value)]
    )
    assert (
        adapt_enum_for_db("UserRole", padawan_value, mock_postgres_session)
        == ENUM_MAPPING[("UserRole", padawan_value)]
    )
    ...
    # Directement avec adapt_enum_for_db
    assert (
        adapt_enum_for_db("UserRole", maitre_value, mock_sqlite_session)
        == ENUM_MAPPING[("UserRole", maitre_value)]
    )
    assert (
        adapt_enum_for_db("UserRole", maitre_value, mock_postgres_session)
        == ENUM_MAPPING[("UserRole", maitre_value)]
    )
    ...
    # Directement avec adapt_enum_for_db
    assert (
        adapt_enum_for_db("UserRole", archiviste_value, mock_sqlite_session)
        == ENUM_MAPPING[("UserRole", archiviste_value)]
    )
    assert (
        adapt_enum_for_db("UserRole", archiviste_value, mock_postgres_session)
        == ENUM_MAPPING[("UserRole", archiviste_value)]
    )
    ...
    # Directement avec adapt_enum_for_db
    assert (
        adapt_enum_for_db("UserRole", gardien_value, mock_sqlite_session)
