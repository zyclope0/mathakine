...
> Updated: 10/04/2026 (lots frontend ciblés fermés jusqu'à QF-04C ; voir puces ci-dessous)
...
- **CSP-HARDEN-08** (closed): global `Content-Security-Policy` in `frontend/next.config.ts` is built by `frontend/lib/security/buildContentSecurityPolicy.ts` ; **production** drops `'unsafe-eval'` from `script-src` (keeps `'unsafe-inline'` — no nonce train in this lot), adds `object-src 'none'`, `form-action 'self'`, `frame-src 'none'`, `upgrade-insecure-requests` ; **development** keeps `'unsafe-eval'` and localhost backend hosts in `connect-src` ; unit tests in `__tests__/unit/lib/security/buildContentSecurityPolicy.test.ts`
...
- **E2E-CORE-06** (minimal, closed): Playwright sur `frontend/__tests__/e2e/` (`auth.spec.ts`, `exercises.spec.ts`, `dashboard.spec.ts`, `badges.spec.ts`, `settings.spec.ts`, `admin.spec.ts`) — **sans** `globalSetup`, **sans** `storageState` global, **sans** `request.post` API dans les specs ; couverture Playwright volontairement limitée aux surfaces invitées : rendu des formulaires auth et contrôle d'accès des pages protégées via redirection vers `/login` ; validation locale forgot-password couverte par le test unitaire `__tests__/unit/app/ForgotPasswordPage.test.tsx` ; suite exécutée en **sériel** (`workers: 1`, `fullyParallel: false`) pour éviter les faux négatifs sur les redirections client temporisées sous `next dev` ; suite admin authentifiée réservée à un futur lot (`describe.skip` dans `admin.spec.ts`) ; **prérequis** : `npx playwright install`, `webServer` lance `npm run dev` ; commande cible : `cd frontend && npx playwright test __tests__/e2e/auth.spec.ts __tests__/e2e/exercises.spec.ts __tests__/e2e/dashboard.spec.ts __tests__/e2e/badges.spec.ts __tests__/e2e/settings.spec.ts __tests__/e2e/admin.spec.ts --project=chromium`
...
- frontend proxy routes (`/api/chat`, `/api/chat/stream`, `/api/exercises/generate-ai-stream`, `/api/challenges/generate-ai-stream`) sont couverts par des tests de handlers Next.js au niveau route ; les deux POST SSE pédagogiques partagent `lib/api/sseProxyRequest.ts` (`proxySseGenerateAiStreamPost`) et `lib/api/proxyForwardHeaders.ts` pour parse JSON, auth cookie, forward headers, garde `body === null` backend et stream
