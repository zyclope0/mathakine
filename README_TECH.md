...
> Updated: 16/04/2026 (lots frontend ciblés fermés jusqu’à ACTIF-04-COVERAGE-02 ; voir puces ci-dessous)
...
- **ACTIF-04-COVERAGE-01 / ACTIF-04-COVERAGE-02** (2026-04-12 → 2026-04-14, closed lot sequence) : **`frontend/vitest.config.ts`** coverage **thresholds** were first rebased on the GitHub Actions frontend baseline (**43 / 36 / 40 / 44** from CI **44.57 / 37.22 / 41.47 / 45.68 %**), then raised again after a new defendable CI measurement on 2026-04-14 (**47.96 / 39.6 / 43.29 / 49.18 %**) to the current **46 / 38 / 42 / 48** via **`floor(CI measured %) − 1`** per axis ; local Windows runs remain non-authoritative versus CI.
...
- **QF-06** (closed): **coverage gates** added to **`frontend/vitest.config.ts`** with an explicit **`coverage.include`** perimeter (`root ts/tsx`, `app`, `components`, `hooks`, `i18n`, `lib`, `messages`) ; thresholds were later lowered to **39/33/37/40** to match a 2026-04-10 CI-style baseline, then recalibrated by **`ACTIF-04-COVERAGE-01`** and **`ACTIF-04-COVERAGE-02`** to the current **46/38/42/48** from the GitHub Actions frontend baselines (**44.57 / 37.22 / 41.47 / 45.68 %** then **47.96 / 39.6 / 43.29 / 49.18 %**, rule **`floor−1`**). Local Windows runs observed higher numbers, but the CI baseline is authoritative.
