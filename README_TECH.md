...
- **AUDIT-QUICKWINS-01** : `server/middleware.py` envoie désormais **`X-XSS-Protection: 0`** ; `.dockerignore` garde les versions Alembic dans l’image ; exceptions **`@next/next/no-img-element`** désormais **`Intentional:`** ou migrées (**PERF-IMG-LOCAL-01** : médailles locales `BadgeCard` / `BadgesProgressTabsSection` → **`next/image`** ; **`ACTIF-02-USERAVATAR-01`** : **`UserAvatar`** → **`next/image`** quand l’URL est couverte par **`remotePatterns`** (incl. **`**.onrender.com`**) + repli **`<img>`** documenté sinon, via **`lib/utils/userAvatarImageSource.ts`** ; **`BadgeIcon`**, **`ChatMessagesView`** encore en `<img>` — **`P1-PERF-03`** / **ACTIF-02** partiellement ouverts).
...
- **ACTIF-02-USERAVATAR-01** (2026-04-11, closed) : **`UserAvatar`** uses **`next/image`** when **`resolveUserAvatarImageDelivery`** matches **`images.remotePatterns`** (including **`**.onrender.com`** added in **`next.config.ts`** alongside existing **`**.render.com`**) ; otherwise **`<img>`** for unbounded DB URLs ; unit tests **`frontend/lib/utils/userAvatarImageSource.test.ts`** ; finding **ACTIF-02** remains open for **`BadgeIcon`** / **`ChatMessagesView`**. See audit § ACTIF-02 / D-02 ; **`.claude/session-plan.md`** § ACTIF-02-USERAVATAR-01.
