...
- **AUDIT-QUICKWINS-01** : `server/middleware.py` envoie désormais **`X-XSS-Protection: 0`** ; `.dockerignore` garde les versions Alembic dans l’image ; exceptions **`@next/next/no-img-element`** : **`Intentional:`** ou migrées (**PERF-IMG-LOCAL-01** ; **`ACTIF-02-USERAVATAR-01`** / **`ACTIF-02-BADGEICON-01`** — hybride **`next/image`** / **`<img>`** via **`lib/utils/nextImageRemoteSource.ts`** ; **`ACTIF-02-CHATMESSAGES-01`** — **`ChatMessagesView`** : **exception produit** **`<img>`** natif pour URLs SSE / **`blob:`** / **`data:`** et layout intrinsèque, voir composant + **`components/chat/ChatMessagesView.test.tsx`**) ; **finding audit ACTIF-02 (D7) fermé**.
...
- **TEST-IRT-SCORES-01** (2026-04-11, closed) : unit tests for **`frontend/hooks/useIrtScores.ts`** in **`frontend/hooks/useIrtScores.test.ts`** (co-localisé, lot **`ACTIF-03-USEIRT-COLOCATE-01`**, 2026-04-12) (`QueryClientProvider`, mocked **`api.get`** on **`/api/diagnostic/status`**, **`useAuth`** via **`vi.hoisted`**) covering direct scores, **`mixte`** / **`fractions`** proxies, profile fallback when **`latest`** is null or type is not IRT-covered, **`has_completed`**, and **`isIrtCovered`** ; no hook refactor, no **`vitest.config.ts`** threshold bump ; advances **[ACTIF-04]** on this hook only.
- **TEST-AI-GENERATOR-01** (2026-04-11, closed) : unit tests for **`frontend/hooks/useAIExerciseGenerator.ts`** in **`frontend/hooks/useAIExerciseGenerator.test.ts`** (co-localisé, lot **`ACTIF-03-USEAI-COLOCATE-01`**, 2026-04-12) (`QueryClientProvider`, partial **`postAiGenerationSse`** mock keeping **`AiGenerationRequestError`** / **`AI_GENERATION_SSE_PATH`**, mocked **`consumeSseJsonEvents`**, real **`dispatchExerciseAiSseEvent`** on the success path, fake timers for the **100 ms** list invalidation) ; **`mockPost.mockReset()`** in **`beforeEach`** to avoid leaking **`mockImplementation`** across cases ; no **`vitest.config.ts`** threshold bump ; advances **[ACTIF-04]** on this hook only.
- **ACTIF-03-USESETTINGS-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useSettingsPageController.ts`** co-localisés dans **`frontend/hooks/useSettingsPageController.test.ts`** (mocks **`useSettings`**, **`useAuth`**, **`api.get`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-USEBADGES-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useBadgesPageController.ts`** co-localisés dans **`frontend/hooks/useBadgesPageController.test.ts`** (FFI-L12 ; mock **`canvas-confetti`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-DASHBOARD-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useDashboardPageController.ts`** co-localisés dans **`frontend/hooks/useDashboardPageController.test.ts`** (mocks hooks dashboard + **`buildDashboardExportSnapshot`**, **`@tanstack/react-query`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-CONTENTLIST-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useContentListPageController.ts`** co-localisés dans **`frontend/hooks/useContentListPageController.test.tsx`** (**`CONTENT_LIST_ORDER`**, **`@/lib/storage`**, mock **`window.scrollTo`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-CHALLENGESOLVER-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useChallengeSolverController.ts`** co-localisés dans **`frontend/hooks/useChallengeSolverController.test.tsx`** (**`Challenge`**, **`ChallengeAttemptResponse`**, **`renderHook`** / **`waitFor`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-EXERCISESOLVER-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useExerciseSolverController.ts`** co-localisés dans **`frontend/hooks/useExerciseSolverController.test.ts`** (mocks **`next/navigation`**, **`next-intl`**, **`useExercise`** / **`useSubmitAnswer`** / **`useIrtScores`** / **`useNextReview`**, **`spacedReviewSession`**, **`INTERLEAVED_STORAGE_KEY`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-MICROHOOKS-COLOCATE-01** (2026-04-12, closed) : lot groupé **5** tests co-localisés sous **`frontend/hooks/`** — **`useAdminContentPageController.test.ts`**, **`useChallengesStats.test.tsx`** (**`@tanstack/react-query`**), **`useChallengeTranslations.test.tsx`** (**`next-intl`**, **`messages/fr.json`**), **`useRecommendationsReason.test.ts`** (**`formatRecommendationReasonDisplay`** depuis **`useRecommendations`**), **`useAccessibleAnimation.test.ts`** (cible **`@/lib/hooks/useAccessibleAnimation`**) ; pas de changement des hooks sources ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-PROGRESS-COLOCATE-01** (2026-04-12, closed) : lot groupé **3** tests progression / liste — **`useCompletedItems.test.tsx`**, **`usePaginatedContent.test.tsx`**, **`useProgressTimeline.test.tsx`** (**`@tanstack/react-query`**, mocks **`api`** / **`localeStore`**) ; pas de changement des hooks sources ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-USECHAT-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/chat/useChat.ts`** co-localisés dans **`frontend/hooks/chat/useChat.test.tsx`** (mock **`streamChat`** depuis **`@/lib/api/chat`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-GUESTCHATACCESS-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/chat/useGuestChatAccess.ts`** co-localisés dans **`frontend/hooks/chat/useGuestChatAccess.test.tsx`** (**`guestChatSession`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; avance **[ACTIF-03]** (co-localisation).
- **ACTIF-03-USEPROFILE-COLOCATE-01** (2026-04-12, closed) : unit tests for **`frontend/hooks/useProfilePageController.ts`** co-localisés dans **`frontend/hooks/useProfilePageController.test.ts`** (mocks **`useProfile`**, **`themeStore`**, **`useAgeGroupDisplay`**) ; pas de changement du hook source ni de **`vitest.config.ts`** ; **`frontend/__tests__/unit/hooks/`** ne porte plus de tests hooks actifs ; **[ACTIF-03]** était alors encore ouvert pour le reste de **`__tests__/unit/`**, avant fermeture complète du finding le 2026-04-14.
- **ACTIF-03-CHAT-COMPONENTS-COLOCATE-01** (2026-04-12, closed) : **`ChatMessagesView.test.tsx`**, **`ChatbotFloating.test.tsx`**, **`ChatbotFloatingGlobal.test.tsx`** moved from **`frontend/__tests__/unit/components/chat/`** to **`frontend/components/chat/`** next to their sources ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-VISUALIZATION-COLOCATE-01** (2026-04-12, closed) : **`VisualizationRenderers.test.tsx`**, **`VisualRenderer.symmetry.test.tsx`** moved from **`frontend/__tests__/unit/components/`** to **`frontend/components/challenges/visualizations/`** next to **`VisualRenderer`**, **`ChessRenderer`**, **`GraphRenderer`**, **`_colorMap`** ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-BADGES-COMPONENTS-COLOCATE-01** (2026-04-12, closed) : **`BadgeCard.test.tsx`**, **`BadgeIcon.test.tsx`**, **`BadgesProgressTabsSection.test.tsx`** moved from **`frontend/__tests__/unit/components/`** (and **`.../components/badges/`**) to **`frontend/components/badges/`** next to their sources ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-DASHBOARD-SMALL-COMPONENTS-COLOCATE-01** (2026-04-13, closed) : **`LevelIndicator.test.tsx`**, **`Recommendations.test.tsx`**, **`SpacedRepetitionSummaryWidget.test.tsx`**, **`SpacedRepetitionSummaryWidget.reviewCta.test.tsx`** moved from **`frontend/__tests__/unit/components/dashboard/`** to **`frontend/components/dashboard/`** next to their sources ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-DASHBOARD-CHARTS-COLOCATE-01** (2026-04-13, closed) : **`CategoryAccuracyChart.test.tsx`**, **`DashboardCategoryRadarChart.test.tsx`** moved from **`frontend/__tests__/unit/components/dashboard/`** to **`frontend/components/dashboard/`** next to their sources ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-APP-BOOTSTRAP-COLOCATE-01** (2026-04-13, closed) : **`Providers.test.tsx`** (`providers`), **`LocaleInitializer.test.tsx`** (`locale`), **`ProtectedRoute.test.tsx`** (`auth`) moved from **`frontend/__tests__/unit/components/`** (and subfolders) to **`frontend/components/providers/`**, **`frontend/components/locale/`**, **`frontend/components/auth/`** next to their sources ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-03-LAYOUT-HEADER-COLOCATE-01** (2026-04-13, closed) : **`Header.test.tsx`** moved from **`frontend/__tests__/unit/components/layout/`** to **`frontend/components/layout/`** next to **`Header.tsx`** ; no test logic change, no **`vitest.config.ts`** change ; advances **[ACTIF-03]** (co-location).
- **ACTIF-04-COVERAGE-01 / ACTIF-04-COVERAGE-02 / ACTIF-04-COVERAGE-03** (2026-04-12 → 2026-04-15, closed lot sequence) : **`frontend/vitest.config.ts`** coverage **thresholds** were first rebased on the GitHub Actions frontend baseline (**43 / 36 / 40 / 44** from CI **44.57 / 37.22 / 41.47 / 45.68 %**), then raised again after a new defendable CI measurement on 2026-04-14 (**47.96 / 39.6 / 43.29 / 49.18 %**) to the current **46 / 38 / 42 / 48** via **`floor(CI measured %) − 1`** per axis ; **ACTIF-04-COVERAGE-03** then added eleven hook suites to defend the raised thresholds without changing the gate values ; local Windows runs remain non-authoritative versus CI.
- **QF-06B-VITEST-ALIGN-01** (2026-04-13, closed) : align **`frontend/package.json`** on the **actual Vitest 4.1.4 family** already resolved in lockfile after the **`@vitest/ui`** dependabot merge (**`vitest`**, **`@vitest/ui`**, **`@vitest/coverage-v8`** all at **`^4.1.4`**) ; no change to **`frontend/vitest.config.ts`** ; documented backlog limited to: observe mock-heavy suites where the sequential **mock/unmock** fix may let us remove defensive workarounds when touched, and require a **new CI measurement** before any future **ACTIF-04** threshold bump ; browser-mode screenshot features and `experimental.*` remain intentionally unused.
- **ACTIF-06-ADMIN-USERS-01** (2026-04-12, closed) : **`frontend/app/admin/users/page.tsx`** is a thin view ; state + handlers live in **`frontend/hooks/useAdminUsersPageController.ts`** (filters, pagination, role/delete dialogs, toasts, **`useAdminUsers`** params) ; tests **`hooks/useAdminUsersPageController.test.tsx`** ; **`useAdminUsers`** and i18n keys unchanged.
- **ACTIF-06-AI-MONITORING-01** (2026-04-12, closed) : **`frontend/app/admin/ai-monitoring/page.tsx`** is a thin view ; state + admin IA hooks wiring live in **`frontend/hooks/useAdminAiMonitoringPageController.ts`** (`days`, aggregated loading/error, **`HARNESS_RUNS_LIMIT`** 25, **`formatWorkloadLabel`**, period options) ; tests **`hooks/useAdminAiMonitoringPageController.test.tsx`** ; **`useAdminAiStats`** / related hooks and i18n keys unchanged ; audit **[ACTIF-06]** **closed** (admin read-heavy pages covered).
- **ACTIF-07-COLORMAP-01** (2026-04-12, closed) : shared **`frontend/components/challenges/visualizations/_colorMap.ts`** (**`VISUALIZATION_COLOR_MAP`**, **`resolveVisualizationColor`**, **`findVisualizationColorInText`**) consumed by **`VisualRenderer.tsx`** and **`ProbabilityRenderer.tsx`** ; tests **`_colorMap.test.ts`** ; audit **[ACTIF-07]** **closed** (DRY color keys for challenge visualizations).
...
- **ACTIF-02-BADGEICON-01** (2026-04-11, closed) : **`BadgeIcon`** — branche **`icon_url`** : **`next/image`** / **`<img>`** selon le même utilitaire ; erreur de chargement → **`useState`** + emoji ; tests **`components/badges/BadgeIcon.test.tsx`**. Audit § ACTIF-02 / D-02 ; **`.claude/session-plan.md`** § ACTIF-02-BADGEICON-01.
- **ACTIF-02-CHATMESSAGES-01** (2026-04-12, closed) : **`ChatMessagesView`** — **pas de `next/image`** : exception **`<img>`** documentée (SSE, schémas **`blob:`** / **`data:`**, **`max-h-64 w-full object-cover`** sans dimensions inventées) ; tests **`components/chat/ChatMessagesView.test.tsx`**. **Finding ACTIF-02** clos. Audit § ACTIF-02 / D-02 ; **`.claude/session-plan.md`** § ACTIF-02-CHATMESSAGES-01.
...
- **QF-06** (closed): **coverage gates** added to **`frontend/vitest.config.ts`** with an explicit **`coverage.include`** perimeter (`root ts/tsx`, `app`, `components`, `hooks`, `i18n`, `lib`, `messages`) ; thresholds were later lowered to **39/33/37/40** to match a 2026-04-10 CI-style baseline, then recalibrated by **`ACTIF-04-COVERAGE-01`** and **`ACTIF-04-COVERAGE-02`** to the current **46/38/42/48** from the GitHub Actions frontend baselines (**44.57 / 37.22 / 41.47 / 45.68 %** then **47.96 / 39.6 / 43.29 / 49.18 %**, rule **`floor−1`**). **`ACTIF-04-COVERAGE-03`** then raised real hook coverage on the same gate without changing those threshold values. Local Windows runs observed higher numbers, but the CI baseline is authoritative.
...
- RÃ©fÃ©rence : [RAPPORT_VALIDATE_TOKEN_RATE_LIMIT_2026-04-07.md](docs/03-PROJECT/archives/IMPLEMENTATION_NOTES_CLOSED_2026-04/RAPPORT_VALIDATE_TOKEN_RATE_LIMIT_2026-04-07.md) (Â§15â€“Â§17). **FFI-L19\*** **clos** ; prioritÃ© active : roadmap **frontend** principale (voir `.claude/session-plan.md`).
