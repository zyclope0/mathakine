...
- **AUDIT-QUICKWINS-01** : `server/middleware.py` envoie désormais **`X-XSS-Protection: 0`** ; `.dockerignore` garde les versions Alembic dans l’image ; les 5 `eslint-disable @next/next/no-img-element` restants dans le frontend ont une justification **`Intentional:`** adjacente au lieu d’une suppression muette.
