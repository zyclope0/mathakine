...
- **AUDIT-QUICKWINS-01** : `server/middleware.py` envoie désormais **`X-XSS-Protection: 0`** ; `.dockerignore` garde les versions Alembic dans l’image ; exceptions **`@next/next/no-img-element`** désormais **`Intentional:`** ou migrées (**PERF-IMG-LOCAL-01** : médailles locales → **`next/image`** ; **`ACTIF-02-USERAVATAR-01`** / **`ACTIF-02-BADGEICON-01`** : **`UserAvatar`** et **`BadgeIcon`** → **`next/image`** si **`resolveNextImageRemoteDelivery`** (`lib/utils/nextImageRemoteSource.ts`) matche **`remotePatterns`**, sinon **`<img>`** ; **`ChatMessagesView`** reste **`<img>`** documenté — **ACTIF-02** ouvert pour le chat uniquement).
...
- **ACTIF-02-BADGEICON-01** (2026-04-11, closed) : **`BadgeIcon`** — branche **`icon_url`** : **`next/image`** / **`<img>`** selon le même utilitaire ; erreur de chargement → **`useState`** + emoji (plus de **`createElement`** / **`appendChild`**) ; tests **`__tests__/unit/components/BadgeIcon.test.tsx`**. **ACTIF-02** reste ouvert pour **`ChatMessagesView`** seul. Audit § ACTIF-02 / D-02 ; **`.claude/session-plan.md`** § ACTIF-02-BADGEICON-01.
