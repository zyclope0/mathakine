...
- **ACTIF-04-COVERAGE-01** (2026-04-12, corrected after CI) : rebaseline **`frontend/vitest.config.ts`** coverage **thresholds** on the **GitHub Actions frontend job baseline** (`ubuntu-latest`, Node **20**, `npx vitest --coverage --reporter=junit --outputFile=./junit.xml --run`) — measured aggregate **All files** in CI failure log : **44.57 %** statements, **37.22 %** branches, **41.47 %** functions, **45.68 %** lines → gates **43 / 36 / 40 / 44** via **`floor(CI measured %) − 1`** per axis ; local Windows runs observed higher coverage and are no longer treated as the source of truth ; no product code change ; audit **[ACTIF-04]** stays **open** for further raises only after new tests + new CI-compatible measurement.
...
- **QF-06** (closed): **coverage gates** added to **`frontend/vitest.config.ts`** with an explicit **`coverage.include`** perimeter (`root ts/tsx`, `app`, `components`, `hooks`, `i18n`, `lib`, `messages`) ; thresholds were later lowered to **39/33/37/40** to match a 2026-04-10 CI-style baseline, then recalibrated by **`ACTIF-04-COVERAGE-01`** (2026-04-12) to the current **43/36/40/44** from the GitHub Actions frontend baseline (**44.57 / 37.22 / 41.47 / 45.68 %**, rule **`floor−1`**). Local Windows runs observed higher numbers, but the CI baseline is authoritative.
