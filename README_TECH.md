...
- **ACTIF-04-COVERAGE-01** (2026-04-12, closed) : rebaseline **`frontend/vitest.config.ts`** coverage **thresholds** from a full local **`npx vitest run --coverage`** (v8, existing **`coverage.include`**) — measured aggregate **All files** : **47.8 %** statements, **39.87 %** branches, **43.19 %** functions, **49.03 %** lines → gates **46 / 38 / 42 / 48** via **`floor(measured %) − 1`** per axis ; no product code change ; audit **[ACTIF-04]** stays **open** for further raises only after new tests + new measurement.
...
- **QF-06** (closed): **coverage gates** added to **`frontend/vitest.config.ts`** with an explicit **`coverage.include`** perimeter (`root ts/tsx`, `app`, `components`, `hooks`, `i18n`, `lib`, `messages`) ; thresholds were later lowered to **39/33/37/40** to match a 2026-04-10 CI-style baseline, then rebased by **`ACTIF-04-COVERAGE-01`** (2026-04-12) to **46/38/42/48** from a measured run (**47.8 / 39.87 / 43.19 / 49.03 %**, **`floor−1`**) ; `npm run test:coverage`, `npm run lint`, and `npx tsc --noEmit` remain green after the rebaseline
