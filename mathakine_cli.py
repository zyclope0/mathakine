...
def setup_parser():
    ...
    run_parser.add_argument("--enhanced", "-e", action="store_true", help="Lancer le serveur amélioré avec interface graphique")
...
def cmd_run(args):
    ...
    # Si l'option enhanced est activée, lancer le serveur amélioré
    if args.enhanced:
        ...
        try:
            ...
            logger.error(f"Erreur lors du chargement du serveur amélioré: {e}")
            logger.error("Assurez-vous que les dépendances nécessaires sont installées")
            return 1
        except Exception as e:
            logger.error(f"Erreur lors du démarrage du serveur amélioré: {e}")
            return 1
    
    # Sinon, lancer le serveur API standard
...
def cmd_setup(args):
    ...
    try:
        returncode, stdout, stderr = run_command([sys.executable, "-m", "pip", "install", "-r", "requirements.txt"])
        if returncode != 0:
