        ...
        {userStats && (
          <PageSection className="space-y-4 animate-fade-in-up-delay-1">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Card className="card-spatial-depth hover:shadow-lg transition-shadow" role="article" aria-label={t('stats.totalPoints')}>
              ...
              <Card className="card-spatial-depth hover:shadow-lg transition-shadow" role="article" aria-label={t('stats.currentLevel')}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-medium text-muted-foreground">
              ...
              <Card className="card-spatial-depth hover:shadow-lg transition-shadow" role="article" aria-label={`${t('stats.jediRank')}: ${rankInfo.title}`}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-medium text-muted-foreground">
                    {t('stats.jediRank')}
                  </CardTitle>
                ...
                <CardContent>
                  <div className="flex items-center gap-3">
                    <span className="text-3xl" aria-hidden="true">{rankInfo.icon}</span>
              ...
              <Card className="card-spatial-depth hover:shadow-lg transition-shadow" role="article" aria-label={`${t('stats.badgesEarned')}: ${earnedCount} sur ${totalCount}`}>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-medium text-muted-foreground">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="w-full bg-muted rounded-full h-3 overflow-hidden" role="progressbar" aria-valuenow={progressPercent} aria-valuemin={0} aria-valuemax={100} aria-label={`Progression: ${progressPercent.toFixed(0)}%`}>
                    <div
                  ...
                  <div className="mt-6 pt-6 border-t border-border">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(gamificationStats.badges_summary.by_category).map(
                        ([category, count]) => (
                          <Badge key={category} variant="outline" className="text-sm" aria-label={`${t(`categories.${category}`)}: ${count as number}`}>
                            {category === 'progression' && '📈'}
                            {category === 'mastery' && '⭐'}
                            {category === 'special' && '✨'}
                            {' '}
                            {t(`categories.${category}`)}: {count as number}
                          </Badge>
                        )
                      )}
                    </div>
        ...
        <PageSection className="space-y-4 animate-fade-in-up-delay-3">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg md:text-xl font-semibold">
              {t('collection.title')} {t('collection.count', { earned: earnedCount, total: totalCount })}
            </h2>
          </div>
          {isLoading ? (
