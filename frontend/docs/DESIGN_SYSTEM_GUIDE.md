...
<PageLayout maxWidth="xl">{/* Contenu de la page */}</PageLayout>;
```
...
<PageSection title="Filtres" description="Filtrez les résultats selon vos préférences">
...
export default function MyPage() {
  ...
  return (
    <ProtectedRoute>
      <PageLayout>
        <PageHeader title="Titre de la Page" description="Description de la page" />

        <PageSection title="Section">
          {isLoading ? (
            <LoadingState />
          ) : data.length === 0 ? (
            <EmptyState title="Aucun résultat" description="Description de l'état vide" />
          ) : (
            <PageGrid>
              {data.map((item) => (
                <ItemCard key={item.id} item={item} />
