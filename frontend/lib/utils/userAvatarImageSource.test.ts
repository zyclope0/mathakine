...
describe("resolveUserAvatarImageDelivery", () => {
  it("routes same-origin paths to next-image", () => {
    expect(resolveUserAvatarImageDelivery("/api/media/avatar.png")).toEqual({
      mode: "next-image",
  ...
  it("trims whitespace before resolving", () => {
    expect(resolveUserAvatarImageDelivery("  /x.jpg  ")).toEqual({
      mode: "next-image",
      src: "/x.jpg",
    });
  });
  ...
  it("keeps protocol-relative URLs on img", () => {
    expect(resolveUserAvatarImageDelivery("//cdn.example.com/a.png")).toEqual({
      mode: "img",
  ...
  it("routes http localhost to next-image", () => {
    expect(resolveUserAvatarImageDelivery("http://localhost:3000/a.png")).toEqual({
      mode: "next-image",
  ...
  it("routes https onrender.com app hosts to next-image", () => {
    expect(resolveUserAvatarImageDelivery("https://my-app.onrender.com/static/1.jpg")).toEqual({
      mode: "next-image",
  ...
  it("routes https *.render.com hosts to next-image", () => {
    expect(resolveUserAvatarImageDelivery("https://assets.render.com/x.png")).toEqual({
      mode: "next-image",
  ...
  it("keeps apex render.com on img (not covered by **.render.com pattern intent)", () => {
    expect(resolveUserAvatarImageDelivery("https://render.com/logo.png").mode).toBe("img");
  });

  it("keeps arbitrary CDNs on img", () => {
    expect(resolveUserAvatarImageDelivery("https://cdn.example.com/a.webp")).toEqual({
      mode: "img",
  ...
  it("returns img for non-URL strings", () => {
    expect(resolveUserAvatarImageDelivery(":::not-a-url").mode).toBe("img");
  });

  it("returns img for empty after trim", () => {
    expect(resolveUserAvatarImageDelivery("   ").mode).toBe("img");
  });
});
