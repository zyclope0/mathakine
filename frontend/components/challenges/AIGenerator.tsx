        ...
        for (const line of lines) {
          ...
          if (trimmedLine.startsWith("data: ")) {
            try {
              ...
              if (isDev)
                console.error(
                  "[AIGenerator] Erreur de parsing SSE:",
                  parseError,
                  "Line:",
                  trimmedLine
                );
      ...
      <CardContent className="space-y-2 pt-0">
        ...
        {generatedChallenge && !isGenerating && (
          <div className="p-3 rounded-lg bg-success/10 border border-success/30">
            <p className="text-xs font-medium text-success mb-1">{t("aiGenerator.success")}</p>
            <p className="text-xs text-muted-foreground mb-2">{generatedChallenge.title}</p>
            <Button
