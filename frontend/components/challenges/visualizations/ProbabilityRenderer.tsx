...
export function ProbabilityRenderer({ visualData, className = '' }: ProbabilityRendererProps) {
  ...
  const itemsData = useMemo(() => {
    ...
    for (const [key, value] of Object.entries(visualData)) {
      ...
      if (typeof value === 'number' && !keyLower.includes('total')) {
        ...
        if (colorMatch || keyLower.match(/^(red|blue|green|yellow|purple|orange|pink|white|black|brown|gray|rouge|bleu|vert|jaune|violet|rose|blanc|noir|marron|gris)_/)) {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
        ...
        <div className="bg-card/50 border border-border rounded-lg p-4">
          ...
          <div className="space-y-3">
            {itemsData.items.map((item, index) => {
              const percentage = itemsData.total > 0 
                ? ((item.count / itemsData.total) * 100).toFixed(1) 
                : 0;
      ...
      {!context && !question && events.length === 0 && probabilities.length === 0 && outcomes.length === 0 && totalOutcomes === 0 && !itemsData && (
        <div className="bg-card/50 border border-border rounded-lg p-4">
          <div className="space-y-2">
