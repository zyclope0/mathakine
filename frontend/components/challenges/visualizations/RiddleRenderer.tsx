...
export function RiddleRenderer({ visualData, className = "" }: RiddleRendererProps) {
  ...
  const plaque =
    rawPlaque && typeof rawPlaque === "object" && rawPlaque !== null
      ? (rawPlaque as { text_lines?: unknown[] })
      : null;
  const plaqueLines =
    plaque?.text_lines && Array.isArray(plaque.text_lines) ? plaque.text_lines : [];
  ...
  const grid =
    visualData.grid || visualData.pattern || visualData.matrix || visualData.rows || null;
            ...
            >
              {grid2D.flat().map((cell: any, index: number) => {
                ...
                return (
                  ...
                  >
                    {isUnknown ? <span className="text-2xl animate-pulse">?</span> : cellValue}
                  </motion.div>
                );
              })}
            </div>
