...
export function Chatbot() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-3 md:space-y-4">
      ...
      <Card className={cn('w-full flex flex-col shadow-lg transition-all', isOpen ? 'h-[500px]' : 'h-16')}>
        <div className="flex items-center justify-between p-4 border-b bg-primary/5">
          <div className="flex items-center gap-2">
            <MessageCircle className="h-5 w-5 text-primary" />
            <h3 className="font-semibold">{t('title')}</h3>
          </div>
          <Button variant="ghost" size="icon" onClick={() => setIsOpen(!isOpen)} aria-label={isOpen ? t('collapseLabel') : t('expandLabel')}>
          ...
          <>
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((message) => (
                <div key={message.id} className={cn('flex items-end gap-2', message.role === 'user' ? 'justify-end' : 'justify-start')}>
                  {message.role === 'assistant' && <MessageCircle className="h-6 w-6 text-primary flex-shrink-0" />}
                  <div className={cn('prose prose-sm max-w-[85%] rounded-lg px-4 py-2', message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted')}>
              ...
              {isLoading && messages[messages.length - 1]?.role === 'user' && (
                <div className="flex justify-start"><div className="bg-muted rounded-lg px-4 py-2"><Loader2 className="h-4 w-4 animate-spin" /></div></div>
              )}
            ...
            {messages.length <= 1 && suggestions.length > 0 && (
              <div className="p-4 border-t">
                <h4 className="text-sm font-semibold mb-2 text-muted-foreground">{t('suggestions')}</h4>
                <div className="flex flex-wrap gap-2">
                  {suggestions.map((s, i) => (
                    <Button key={i} variant="outline" size="sm" onClick={() => handleSend(s)} className="text-xs">{s}</Button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="p-4 border-t bg-background">
              <div className="flex gap-2">
                ...
                />
                <Button onClick={sendInputMessage} disabled={!input.trim() || isLoading} size="icon" aria-label={t('sendButton')}>
