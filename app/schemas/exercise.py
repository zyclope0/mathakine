...
class ExerciseBase(BaseModel):
    ...
    question: str = Field(
        ..., min_length=5, description="Question de l'exercice (min 5 caractères)"
    )
    correct_answer: str = Field(..., description="Réponse correcte à la question")
    choices: Optional[List[str]] = Field(
        None, description="Options pour les questions à choix multiples"
    )
    explanation: Optional[str] = Field(None, description="Explication de la solution")
    hint: Optional[str] = Field(None, description="Indice pour aider l'élève")
    ...
    def validate_choices(cls, v, info):
        if v is not None:
            ...
            if "correct_answer" in values and values["correct_answer"] not in v:
                raise ValueError(
                    "La réponse correcte doit être présente dans les choix"
                )
    ...
    def validate_question(cls, v):
        ...
        if not any(mark in v for mark in ["?", ":", "!", "."]):
            raise ValueError(
                "La question doit contenir une ponctuation finale (?, :, !, .)"
            )
        return v
...
class ExerciseCreate(ExerciseBase):
    ...
    audio_url: Optional[str] = Field(None, description="URL audio pour accessibilité")
