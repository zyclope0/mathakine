...
class ProgressBase(BaseModel):
    ...
    exercise_type: str = Field(
        ..., description="Type d'exercice (addition, soustraction, etc.)"
    )
    ...
    average_time: Optional[float] = Field(
        None, ge=0.0, description="Temps moyen pour résoudre (secondes)"
    )
    completion_rate: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="Taux de complétion (%)"
    )
    ...
    def correct_not_greater_than_total(cls, v, info):
        ...
        if "total_attempts" in values and v > values["total_attempts"]:
            raise ValueError(
                "Le nombre de tentatives réussies ne peut pas dépasser le nombre total de tentatives"
            )
        return v
...
class ProgressUpdate(BaseModel):
    ...
    def correct_not_greater_than_total(cls, v, info):
        ...
        if (
            v is not None
            and "total_attempts" in values
            and values["total_attempts"] is not None
    ...
    def highest_streak_not_less_than_streak(cls, v, info):
        ...
        if (
            v is not None
            and "streak" in values
            and values["streak"] is not None
            and v < values["streak"]
        ):
            raise ValueError(
                "La meilleure série ne peut pas être inférieure à la série actuelle"
            )
        return v
...
class UserProgressSummary(BaseModel):
    ...
    overall_mastery: float = Field(
        ..., ge=0.0, le=5.0, description="Niveau de maîtrise global (0-5)"
    )
...
class ProgressDetail(ProgressResponse):
    ...
    recent_attempts: List[RecentAttempt] = Field(
        default_factory=list, description="Tentatives récentes"
    )
