...
class ResponseMeta(BaseModel):
    ...
    error_code: Optional[ErrorCode] = Field(
        None, description="Code d'erreur si applicable"
    )
    pagination: Optional[PageInfo] = Field(
        None, description="Information de pagination si applicable"
    )
    debug_info: Optional[Dict[str, Any]] = Field(
        None, description="Informations de débogage (en mode debug uniquement)"
    )
...
class ErrorResponse(BaseModel):
    ...
    errors: Optional[List[ValidationError]] = Field(
        None, description="Liste détaillée des erreurs de validation"
    )
...
class HealthCheck(BaseModel):
    ...
    dependencies: Dict[str, str] = Field(
        ..., description="État des dépendances externes"
    )
