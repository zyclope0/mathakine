...
class User(Base):
    ...
    role = Column(
        Enum(UserRole, name="userrole", create_type=False), default=UserRole.PADAWAN
    )
    ...
    last_activity_date = Column(
        Date, nullable=True
    )  # Dernier jour calendaire avec activité
    ...
    created_exercises = relationship(
        "Exercise", back_populates="creator", cascade="all, delete-orphan"
    )
    ...
    created_logic_challenges = relationship(
        "LogicChallenge", back_populates="creator", cascade="all, delete-orphan"
    )
