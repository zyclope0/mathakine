...
class Recommendation(Base):
    ...
    challenge_id = Column(
        Integer,
        ForeignKey("logic_challenges.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    recommendation_type = Column(
        String(20), default="exercise", nullable=False, index=True
    )
    ...
    )  # 1-10 (plus c'est élevé, plus c'est prioritaire)
    reason = Column(
        Text, nullable=True
    ...
    shown_count = Column(
        Integer, default=0
    )  # Nombre de fois que cette recommandation a été affichée
    clicked_count = Column(
        Integer, default=0
    )  # Nombre de fois que l'utilisateur a cliqué dessus
