...
Source de vérité par défaut : :data:`DEFAULT_CHALLENGES_AI_MODEL` (``o4-mini``).
...
3. :data:`CHALLENGE_MODEL_BY_TYPE` — aujourd'hui ``o4-mini`` pour chaque type (carte réellement utilisée
   lorsque aucun override global n'est défini).
4. :data:`DEFAULT_CHALLENGES_AI_MODEL` si le type est inconnu.

**Fallback** (appel HTTP non stream **uniquement** si le stream principal est o-series et renvoie un
...
pas ce fallback (branche inactive hors famille o-series).

La matrice de **capacités** par famille (o1 / o-series / gpt-5.x / chat) reste dans
...
DEFAULT_CHALLENGES_AI_MODEL: Final[str] = "o4-mini"

# Secours si le stream o-series ne renvoie aucun contenu : modèle chat JSON (allowlist).
...
# Produit actuel : homogène ``o4-mini`` pour tous les types (lisible, gouvernable).
...
def resolve_challenge_ai_model(challenge_type: str) -> str:
    ...
    Override global (ops) > override legacy > carte par type > défaut ``o4-mini``.
    """
...
def build_challenge_ai_stream_kwargs(
    *,
    ...
    ``max_completion_tokens`` pour o1 / o-series / GPT-5 ; ``reasoning_effort`` pour o-series et GPT-5 ;
