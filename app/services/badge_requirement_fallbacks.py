...
def _make_success_rate_checker(
    ...
    def _check(
        ...
        rate = requirements.get("success_rate", rate_default)
        min_attempts = requirements.get("min_attempts", min_attempts_default)
        return _check_success_rate(db, user_id, rate, min_attempts)

    return _check


def _check_success_rate(
    db: Session, user_id: int, min_success_rate: float, min_attempts: int
) -> bool:
    stats = db.execute(
        text("""
            SELECT
