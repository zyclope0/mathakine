...
Voir AUDIT_SECURITE_APPLICATIVE_2026-02.md §10.
"""
...
class RateLimiter:
    ...
    def check_rate_limit(
        self, user_id: int, max_per_hour: int = 10, max_per_day: int = 50
        ...
        if len(daily_timestamps) >= max_per_day:
            return (
                False,
                f"Limite journalière atteinte ({max_per_day} générations/jour)",
            )
    ...
    def _cleanup_old_entries(self):
        ...
        logger.debug(
            f"Nettoyage rate limiter: {len(self._user_generation_counts)} utilisateurs actifs"
        )
