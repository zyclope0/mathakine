...
def upgrade() -> None:
    ...
    if "user_sessions" not in existing:
        op.create_table(
            ...
            sa.Column(
                "device_info", postgresql.JSON(astext_type=sa.Text()), nullable=True
            ),
    ...
    op.execute(
        sa.text("CREATE INDEX IF NOT EXISTS ix_user_sessions_id ON user_sessions (id)")
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text("CREATE INDEX IF NOT EXISTS ix_notifications_id ON notifications (id)")
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text("CREATE INDEX IF NOT EXISTS idx_users_avatar_url ON users (avatar_url)")
    )
