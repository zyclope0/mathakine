...
def upgrade() -> None:
    ...
    age_group_exists = index_exists(
        connection, "ix_exercises_age_group", "exercises"
    ) or index_exists(connection, "idx_exercises_age_group", "exercises")
    if not age_group_exists:
        op.create_index(
            "ix_exercises_age_group", "exercises", ["age_group"], unique=False
        )
        print("  [CREATED] ix_exercises_age_group")
    else:
        print(
            "  [SKIPPED] ix_exercises_age_group (or idx_exercises_age_group already exists)"
        )
    ...
    is_archived_exists = index_exists(
        connection, "ix_exercises_is_archived", "exercises"
    ) or index_exists(connection, "idx_exercises_is_archived", "exercises")
    if not is_archived_exists:
        op.create_index(
            "ix_exercises_is_archived", "exercises", ["is_archived"], unique=False
        )
    ...
    archived_age_exists = index_exists(
        connection, "ix_exercises_archived_age", "exercises"
    ) or index_exists(connection, "idx_exercises_archived_age", "exercises")
...
def downgrade() -> None:
    ...
    if index_exists(connection, "ix_exercises_archived_age", "exercises"):
        ...
        print(
            "  [SKIPPED] ix_exercises_archived_age (not found or not created by this migration)"
        )
    ...
    if index_exists(connection, "ix_exercises_is_archived", "exercises"):
        ...
        print(
            "  [SKIPPED] ix_exercises_is_archived (not found or not created by this migration)"
        )
    ...
    if index_exists(connection, "ix_exercises_age_group", "exercises"):
        ...
        print(
            "  [SKIPPED] ix_exercises_age_group (not found or not created by this migration)"
        )
