...
def print_results(needs_attention, needs_fix):
    ...
    print(f"  Fichiers nécessitant une correction d'encodage: {color_text(len(needs_fix)
        , Fore.RED)}")

    if needs_attention:
        print(f"\n{color_text('Fichiers nécessitant une attention (encodage non-UTF-8 ou faible confiance):'
            , Fore.YELLOW)}")
    ...
    if needs_fix:
        print(f"\n{color_text('Fichiers nécessitant une correction d\'encodage (caractères accentués mal encodés):'
            , Fore.RED)}")
